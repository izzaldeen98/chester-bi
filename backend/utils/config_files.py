...
class BaseStorage(ABC):
    @abstractmethod
    async def upload_file(self, file: BytesIO, account_id: int, path: str, file_name: str) -> str:
        """Uploads a file and returns its public path or URL string"""
        pass

    @abstractmethod
    async def delete_file(self, account_id: int, path: str, file_name: str) -> bool:
        """Deletes a file from storage"""
        pass

    @abstractmethod
    async def get_file(self, account_id: int, path: str, file_name: str) -> BytesIO:
        """Gets a file from storage"""
        pass

    @abstractmethod
    async def rollback_file(self, account_id: int, path: str, file_name: str) -> bool:
...
class S3Storage(BaseStorage):
    ...
    async def upload_file(self, file: BytesIO, account_id: int, path: str, file_name: str) -> str:
        s3_key = f"{account_id}/{path}/{file_name}"
    ...
    async def delete_file(self, account_id: int, path: str, file_name: str) -> bool:
        try:
            key = f"{account_id}/{path}/{file_name}"
...
class LocalStorage(BaseStorage):
    ...
    async def upload_file(self, file: BytesIO, account_id: int, path: str, file_name: str) -> str:
        # Create directories based on your strategy definitions
        target_dir = os.path.join(self.base_dir, f"{path}/{account_id}")
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Local disk save failed: {str(e)}")

    async def get_file(self, account_id: int, path: str, file_name: str) -> BytesIO:
        file_path = os.path.join(self.base_dir, f"{path}/{account_id}", file_name)
    ...
    async def delete_file(self, account_id: int, path: str, file_name: str) -> bool:
        file_path = os.path.join(self.base_dir, f"{path}", file_name)
